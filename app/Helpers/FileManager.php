...
class FileManager
{
        ...
        if ($source) {
            ...
            if ($decoded_source) {
                if (isset($decoded_source->url)) {
                    ...
                    return $decoded_source->disk === 's3'
                        ? self::s3_url('s3', $decoded_source->path) //\Storage::disk('s3')->temporaryUrl(ltrim($decoded_source->path, '/'), carbon::now()->addHours(6))
                        : $decoded_source->url;
                } else {
    ...
    static function s3_url($disk_driver, $final_path)
    {
        ...
        if ($disk['driver'] === 's3') {
            return \Storage::disk('s3')->url(ltrim($final_path, '/'));
